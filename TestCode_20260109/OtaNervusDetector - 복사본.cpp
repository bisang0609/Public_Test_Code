...
void Detector::detectLine(Mat& in, Mat& out)
{
    ...
    // gray scale
    Mat gray;
#if 0 //dual referenceChannelBlack
    if (!referenceChannelBlack.empty())
    {
#if 0
        Mat inLab;
        cvtColor(blur, inLab, COLOR_BGR2Lab);
        vector<Mat> inChannels;
        split(inLab, inChannels);

        Mat diff;
        // 1. 차분 수행 (입력 - 두꺼워진 레퍼런스)
        absdiff(inChannels[0], referenceChannelBlack[0], diff);

        // 2. [중요/추가] 노이즈 게이트 (Noise Gate)
        // 차분 결과값이 30보다 작은(희미한 고스트) 픽셀은 모두 검은색(0)으로 만듦
        // 반사광 찌꺼기를 확실히 제거하는 역할
        threshold(diff, diff, 30, 255, THRESH_TOZERO);

        diff.copyTo(gray);

        // 디버깅: 이 단계의 이미지를 확인해보면 반사광이 깔끔하게 사라졌는지 알 수 있습니다.
        save_dbg_img(gray, "clean_diff", 3);
#elif 0
        // 1. 레퍼런스에서 '반사광 영역'만 뽑아내기 (Mask 생성)
        Mat mask;
        // L채널(밝기)이 30 이상인 곳을 반사광으로 간주
        threshold(referenceChannelBlack[0], mask, 30, 255, THRESH_BINARY);

        // 마스크 살짝 넓히기 (경계선 노이즈 방지)
        Mat kernel = getStructuringElement(MORPH_RECT, Size(5, 5));
        ...
        // 3. [핵심] 반사광 자리를 '흰색(255)'으로 덮어쓰기
        // 이렇게 하면 THRESH_BINARY_INV가 이 영역을 '배경'으로 취급하여 무시합니다.
        gray.setTo(Scalar(255), mask);

        // (확인용) 이 이미지를 열어보시면 반사광 자리가 하얗게 메꿔져 있을 겁니다.
        save_dbg_img(gray, "masked_white", 3);
#elif 1
        // 1. 마스크 생성 (반사광 위치 찾기)
        cvtColor(blur, gray, COLOR_BGR2GRAY);

        Mat mask;
        double maskThresh = 200.0; // 기준값을 30 -> 50 등으로 높여볼 예정
        threshold(referenceChannelBlack[0], mask, maskThresh, 255, THRESH_BINARY);
        //threshold(referenceChannelBlack[0], mask, 30, 255, THRESH_BINARY);
        save_dbg_img(mask, "debug_mask_check", 3);

        // 2. 마스크 확장 (반사광보다 살짝 넓게 덮어야 깔끔함)
        Mat kernel = getStructuringElement(MORPH_RECT, Size(15, 15)); // 넉넉하게 확장
        dilate(mask, mask, kernel);

        // 3. [핵심] 반사광 자리를 '127 (중간 회색)'으로 칠하기
        // 0(검정)이나 255(흰색)은 부작용이 있지만, 127은 피부톤과 비슷해 무시됨
        gray.setTo(Scalar(127), mask);

        // (확인용) 회색으로 땜질된 이미지가 저장됩니다.
        save_dbg_img(gray, "gray_neutralized", 3);
#endif
    }
    else
    {
        // 레퍼런스 파일이 없을 경우, 기존 방식대로 단순 흑백 변환
        cvtColor(blur, gray, COLOR_BGR2GRAY); // 또는 RGB2GRAY
    ...
    if (!referenceChannelBlack.empty())
        {
            // (1) 레퍼런스에서 반사광 마스크 생성
            Mat mask;
            // 밝기가 30 이상인 곳을 찾음 (반사광 위치)
            threshold(referenceChannelBlack[0], mask, 30, 255, THRESH_BINARY);

            // (2) 마스크 확장 (경계선까지 확실히 덮기 위해)
            Mat kernel = getStructuringElement(MORPH_RECT, Size(5, 5));
            dilate(mask, mask, kernel);

            // (3) 이진화 결과(bin)에서 마스크에 해당하는 부분을 '검은색(0)'으로 칠함
            // setTo(색상, 마스크): 마스크가 흰색인 곳만 0으로 바꿈
            bin.setTo(0, mask);

            // (확인용) 반사광 자리가 까맣게 구멍이 뚫렸는지 확인해보세요.
            save_dbg_img(bin, "bin_masked_black", 3);
        }
#endif
#ifdef ADD_VIEW
    m_bin = bin.clone();
